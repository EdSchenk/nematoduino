...
// make normBodyTotal global for sound effect
int32_t normBodyTotalGlob;
...
//==> int16_t* CurrMuscleState = malloc((N_NTOTAL - N_MAX) * sizeof(int16_t));//geeft warning
int16_t* CurrMuscleState = (int16_t*)malloc((N_NTOTAL - N_MAX) * sizeof(int16_t));
//==> int16_t* NextMuscleState = malloc((N_NTOTAL - N_MAX) * sizeof(int16_t));//geeft warning
...
//==>    uint8_t* IdleCycles = malloc(N_MAX*sizeof(uint8_t));//geeft warning
...
void ActivateMuscles() {
  ...
  //debug  Serial.println(4 * leftNeckTotal);
  //debug  Serial.println(4 * rightNeckTotal);
...
void setup() {
  ...
  // Loop until pressed BUTTON
  /* debug button uitgeschakeld
    while (true) {
    if (ButtonPress()) {
      break;
    }
